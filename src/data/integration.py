...
class DatasetIntegrator:
    def __init__(self, config_path: str):
        ...
        self.output_path = Path(self.config["paths"]["integrated"])
    ...
    def _filter_isic_melanoma(self):
        isic_path = Path(self.config["datasets"]["isic"]["metadata"])

        df = pd.read_csv(isic_path)

        melanoma = df[
            df["diagnosis"].str.lower().str.contains("melanoma", na=False)
        ].copy()
        ...
        return melanoma[["image_id", "image_path", "fitzpatrick_label"]], len(df)

    def _filter_fitzpatrick_melanoma(self):
        fitz_path = Path(self.config["datasets"]["fitzpatrick"]["metadata"])

        df = pd.read_csv(fitz_path)

        melanoma = df[df["label"].str.lower().str.contains("melanoma", na=False)].copy()
        ...
        melanoma["fitzpatrick_label"] = melanoma["fitzpatrick_scale"]

        return melanoma[["image_id", "image_path", "fitzpatrick_label"]], len(df)

    def _generate_report(self, isic_melanoma, isic, fitz_melanoma, fitz):
        report = {
            "isic": {"original_samples": isic, "melanoma_samples": isic_melanoma},
            "fitzpatrick17k": {
                "original_samples": fitz,
                "melanoma_samples": fitz_melanoma,
            },
            "integrated_dataset": {"total_samples": fitz_melanoma + isic_melanoma},
        }
    ...
    def run(self):
        ...
        integrated = pd.concat([isic, fitzpatrick], ignore_index=True)

        output_file = self.output_path / "metadata.csv"
...
if __name__ == "__main__":
    integrator = DatasetIntegrator("configs/config.yaml")
